...
export default function HomeScreen() {
  ...
  const getExercicios = async (date: Date, idPatient: string = pacientId) => {
    ...
    const response = await supabaseService.getExercicies(
      idPatient,
      formattedDate
    );
  ...
  return (
    <SafeAreaProvider>
      <SafeAreaView style={styles.container} edges={["top"]}>
        ...
        {exercicies?.length !== 0 && !loadingExercises && exercicies.filter((x: any) => !x.feito).length ? (
          <Pressable style={styles.button} onPress={handleTimer}>
            <Text style={styles.buttonText}>
              { exercicies.filter((x: any) => x.feito).length > 0 ? "Continuar" : "Iniciar" }
            </Text>
          </Pressable>
        ) : null}
        {exercicies?.length !== 0 && !loadingExercises && exercicies.filter((x: any) => x.feito).length === exercicies?.length ? (
          <Text style={{...styles.exerciseText, marginBottom: 40, fontWeight: 'bold'}}>Todos os exercícios foram finalizados</Text>
        ) : null}
      </SafeAreaView>
    </SafeAreaProvider>
  );
